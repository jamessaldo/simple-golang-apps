...
func TestSaverPost_Success(t *testing.T) {
	...
	postApp.SavePostFn = func(*entity.Post) (*entity.Post, map[string]string) {
		return &entity.Post{
			ID:          1,
			UserID:      1,
			Title:       "Post title",
			Description: "Post description",
			PostImage:   "dbdbf-dhbfh-bfy34-34jh-fd.jpg",
		}, nil
	}
	...
	fileWriter, err := multipartWriter.CreateFormFile("post_image", "amala.jpg")
	if err != nil {
	...
	assert.EqualValues(t, post.PostImage, "dbdbf-dhbfh-bfy34-34jh-fd.jpg")
}
...
func TestGetAllPost_Success(t *testing.T) {
	...
	postApp.GetAllPostFn = func() ([]entity.Post, error) {
		return []entity.Post{
			{
				ID:          1,
				UserID:      1,
			...
			},
			{
				...
				PostImage:   "dbdbf-dhbfh-bfy34-34jh-fd-second.jpg",
			},
		}, nil
	}
...
func TestGetPostAndCreator_Success(t *testing.T) {
	...
	//Return Post to check for, with our mock
	postApp.GetPostFn = func(uint64) (*entity.Post, error) {
		return &entity.Post{
			ID:          1,
			UserID:      1,
			Title:       "Post title",
	...
	r := gin.Default()
	r.GET("/post/:post_id", f.GetPostAndCreator)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	assert.EqualValues(t, post["title"], "Post title")
	assert.EqualValues(t, post["description"], "Post description")
	assert.EqualValues(t, post["post_image"], "dbdbf-dhbfh-bfy34-34jh-fd.jpg")
...
func TestUpdatePost_Success_With_File(t *testing.T) {
	...
	//Mocking The Post return from db
	postApp.UpdatePostFn = func(*entity.Post) (*entity.Post, map[string]string) {
		return &entity.Post{
			ID:          1,
			UserID:      1,
			Title:       "Post title updated",
			Description: "Post description updated",
			PostImage:   "dbdbf-dhbfh-bfy34-34jh-fd-updated.jpg",
		}, nil
	}
	...
	//Initialize the file field
	fileWriter, err := multipartWriter.CreateFormFile("post_image", "new_meal.jpeg")
	if err != nil {
		t.Errorf("Cannot write file: %s\n", err)
	}
	//Copy the actual content to the file field's writer, though this is not needed, since files are sent to DigitalOcean
	...
	r := gin.Default()
	r.PUT("/post/:post_id", f.UpdatePost)
	req.Header.Set("Authorization", tokenString)
	req.Header.Set("Content-Type", multipartWriter.FormDataContentType()) //this is important
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
	...
	assert.EqualValues(t, post.PostImage, "dbdbf-dhbfh-bfy34-34jh-fd-updated.jpg")
}
...
func TestUpdatePost_Success_Without_File(t *testing.T) {
	...
	postApp.GetPostFn = func(uint64) (*entity.Post, error) {
		return &entity.Post{
			...
			Title:       "Post title",
			Description: "Post description",
			PostImage:   "dbdbf-dhbfh-bfy34-34jh-fd-old-file.jpg",
		}, nil
	}
	//Mocking The Post return from db
	postApp.UpdatePostFn = func(*entity.Post) (*entity.Post, map[string]string) {
	...
	assert.EqualValues(t, post.Title, "Post title updated")
	assert.EqualValues(t, post.Description, "Post description updated")
	assert.EqualValues(t, post.PostImage, "dbdbf-dhbfh-bfy34-34jh-fd-old-file.jpg")
}
...
func TestUpdatePost_Invalid_Data(t *testing.T) {
	...
	for _, v := range samples {
		...
		r := gin.Default()
		r.POST("/post/:post_id", f.UpdatePost)
...
func TestDeletePost_Success(t *testing.T) {
	...
	r := gin.Default()
	r.DELETE("/post/:post_id", f.DeletePost)
	req.Header.Set("Authorization", tokenString)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
