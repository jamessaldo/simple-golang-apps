...
func CIBuild() (*gorm.DB, error) {
	var err error
	DBURL := fmt.Sprintf("host=%s port=%s user=%s dbname=%s sslmode=disable password=%s", "127.0.0.1", "5432", "steven", "nc-two-test", "password")
...
func LocalDatabase() (*gorm.DB, error) {
	...
	err = conn.DropTableIfExists(&entity.User{}, &entity.Post{}).Error
	if err != nil {
		return nil, err
	}
